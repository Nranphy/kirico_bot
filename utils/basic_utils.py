...
def kirico_logger(level:str, title:str, detail:str):
    '''
    在后台输出日志
    :param level: 日志等级，有 debug, info, warning, error, critical，大小写不限.
    :param title: 日志标题
    :param detail: 日志内容
    '''
    level = level.upper()
    if level not in ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']:
        logger = logger_wrapper("日志工具")
        logger("error", "调用 kirico_logger 时日志等级设置错误。")
        return
    log = logger_wrapper(title)
    log(level, detail)
    ...
    def __post_init__(self):
        ...
        for k,v in kirico_plugin_metadata.items():
            if not hasattr(self,k):
                ...
                raise ValueError("插件元信息错误，extra 中含有已有属性。")
