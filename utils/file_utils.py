...
def set_cd(name:str, second:int, user_id:Optional[Union[str,int]] = None, group_id:Optional[Union[str,int]] = None, global_cd:bool=False):
    '''
    为name功能增加CD
    :param name: 用以区别的功能名
    ...
    :param global_cd: 是否添加全局CD
    :若user_id与group_id均留空，则强制添加全局冷却；两者均提供则均增加冷却
    '''    
    ...
    if (not (user_id or group_id)) or global_cd:
    ...
    if user_id:
        if check_file(user_path):
            ...
            user_cd_data = {}
        user_cd_data[str(user_id)] = available_timestamp
    ...
    if group_id:
        if check_file(group_path):
            ...
            group_cd_data = {}
        group_cd_data[str(group_id)] = available_timestamp
...
def check_cd(name:str, user_id:Optional[Union[str,int]] = None, group_id:Optional[Union[str,int]] = None):
    ...
    if global_path.is_file():
        ...
        if now_date < available_timestamp:
            return False
    if group_path.is_file() and group_id:
        with open(group_path, "r") as f:
            available_timestamp = json.load(f).get(str(group_id), 0)
        if now_date < available_timestamp:
            return False
    if user_path.is_file() and user_id:
        with open(user_path, "r") as f:
            available_timestamp = json.load(f).get(str(user_id), 0)
        if now_date < available_timestamp:
            return False
    return True