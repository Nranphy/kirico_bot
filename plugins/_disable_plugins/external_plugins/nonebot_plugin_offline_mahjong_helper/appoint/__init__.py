...
async def mahjong_appoint_guide_process(bot:Bot, event:Event):
...
async def mahjong_appoint_create_process(bot:Bot, event:Event):
...
async def mahjong_appoint_join_process(bot:Bot, event:Event, arg:Message=CommandArg()):
...
async def mahjong_appoint_quit_process(bot:Bot, event:Event, arg:Message=CommandArg()):
    table = arg.extract_plain_text().strip()
    if not table:
